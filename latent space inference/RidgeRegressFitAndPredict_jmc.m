...
% The original RegressFitAndPredict function syntax results in all lambdas
% being tested on the held out outer fold (this is likely because the
% cvParameter (which is a vector of lambdas in the ridge regression case)
% is in the case of RRR actually a vector over the range of number of
...
% predictive dimensions; here there is no way of knowing which was the
...
% across folds). It is biased to simply select the lambda-model leading to
% best performance on the held out test fold(s); rather, the best lambda
% should be selected via inner CV within the current outer training fold
% and then tested on the held out outer training fold. This modification
% selects an optimal lambda-model within the current outer training fold
% and uses it to obtain a mapping matrix, B, which is then passed to the
% RegressPredict function, which calculates the loss. This must be done in
% a separately named function (and not as a direct to modification to
